...
while true; do
	ping -c1 $(echo "$tele_url" | cut -d '/' -f 3) 2>&1 > /dev/null && {
		...
		for ((i=0; i<"$updates_count"; i++)); do
			(
			date +%F-%T >> $path_to_log
			echo "$updates" | jq ".result[$i]" >> $path_to_log
			...
			case $message_text in
				...
				'hashtag'*)
					...
					case $message_text in
						'hashtag -s'*)
							...
							if (( $(echo $content_text | wc -m) < 18 )); then
								if [[ $content_text =~ ^[[:alnum:]]+$ ]]; then
									if [[ ! "$(sqlite3 $path_to_db <<< "select user_id from hashtags where user_id='"$user_id"' AND hashtag='"$content_text"';")" ]]; then
						...
						;;
						'hashtag -u'*)
							sqlite3 $path_to_db <<< "delete from hashtags where user_id='"$user_id"' AND hashtag='"$content_text"';"
							send "$chat_id" "$(message_id)" "You have unsubscribed from tag $content_text."
						;;
						'hashtag -l'*)
				...
				;;
				'sources'*)
					send "$chat_id" "$(message_id)" "https://github.com/vlakam/tgbashbot"
				;;
				'/me '*)
					reply_id
